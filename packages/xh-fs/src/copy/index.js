...
module.exports = (
  source,
  destination,
  { concurrency = (os.cpus().length || 1) * 2, ...options } = {},
) => {
  ...
  const promise = (async () => {
    ...
    let sources = files.map(
      (sourcePath) => new SourceFile(sourcePath, preprocessSourcePath(sourcePath, options)),
    );

    if (options.filter !== undefined) {
      const filteredSources = await pFilter(sources, options.filter, {
        concurrency: 1024,
      });
