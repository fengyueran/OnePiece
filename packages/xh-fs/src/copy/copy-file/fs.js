...
exports.utimes = (path, atime, mtime) =>
  utimes(path, atime, mtime).catch((error) => {
    throw new CpFileError(`utimes \`${path}\` failed: ${error.message}`, error);
  });

exports.chmod = (path, mode) =>
  chmod(path, mode).catch((error) => {
    throw new CpFileError(`chmod \`${path}\` failed: ${error.message}`, error);
  });

exports.chown = (path, uid, gid) =>
  chown(path, uid, gid).catch((error) => {
...
exports.copyFileSync = (source, destination, flags) => {
  try {
    ...
    throw new CpFileError(
      `Cannot copy from \`${source}\` to \`${destination}\`: ${error.message}`,
      error,
    );
  }
};
